...
This readme contains all of the documentation on the Ingres Ruby drover and
ActiveRecord adapter. ActiveRecord is the standard database
access interface used by Ruby on Rails (RoR or Rails) applications.  Please
review this README.rdoc before building or installing this software. We encourage
...
To summarize, the ActiveRecord interface provides an object interface while the driver
...
supported by Ingres and Ruby, including:
...
* C compiler (for example, GNU/C on Linux and UNIX, or Microsoft Visual Studio
  ...
  Note: It's not possible to build Ruby extensions with newer versions of the
  Visual Studio C compiler without making changes to the Ruby header files.
* The Ingres Ruby ActiveRecord adapter source code and driver binary. The source code for
...
intended for use with the Ingres ActiveRecord adapter. The API may change in the 
future. It is, however, useful for testing Ingres connectivity and functionality.
Ruby program must have a <tt>require 'Ingres'</tt> statement to load in driver file.

=== Driver Methods

The table below lists the methods available in the Ingres Class. Click on the
method name for more information and examples on usage.

<table>
<tr><td>*Method*<td>*Description*
<tr><td>Ingres.new()<td>To instantiate a new Ingres object
...
<tr><td>Ingres.connect()<td>Connect to the specified database
<tr><td>Ingres.current_database()<td>Returns name of the current database connected to
...
<tr><td>Ingres.set_debug_flag()<td>Enable/disable debugging
<tr><td>Ingres.set_environment()<td>Change environmental settings affecting
<tr><td>Ingres.tables()<td>Returns list of all tables in the current database
query behaviour
</table>

== Ingres ActiveRecord Adapter

See Ruby ActiveRecord documentation: http://ar.rubyonrails.com for more
information.
...
A binary version of the driver is provided for Windows (Ingres.so) at
http://esd.ingres.com. If using the provided binary, skip to the install step below.
...
3. Compile and link the driver. Type <tt>make</tt> on Linux and UNIX or
   ...
   * On Windows, <tt>Ingres.so</tt> is a DLL, but is nevertheless suffixed with ".so"
     rather than ".dll".
...
installation. See http://docs.rubygems.org/read/chapter/3 for information on
installing the RubyGems package.

There are several steps required to install the Ingres ActiveRecord adapter into the Ruby
ActiveRecord installation.

Note: If only using the Ingres Ruby driver interface (not ActiveRecord), installation of
...
3. If you do not wish to run the Rails unit tests against Ingres you can skip this
   ...
   the supplied <tt>connection.rb</tt> file to define database name, user ID, password
   information.
...
A Cookbook Tutorial can be accessed from the Rails website 
(http://rubyonrails.org/docs) under the Tutorials section. Selection Rolling with Ruby
...
1. Install the Ingres Ruby ActiveRecord adapter and driver into InstantRails per the
...
If you would like to get a copy of the Frequent Flyer demo, please email
...
As this product is still in beta status, issues specific to the Ingres Ruby
ActiveRecord adapter and driver should be posted on the Ingres forum 
http://community.ingres.com/forum/ruby-driver-ruby-rails-adapter/. Users with an
active support contract can raise an issue via their local support center.
You can find more information on using the Ingres Ruby driver and ActiveRecord 
bindings at http://community.ingres.com/wiki/Ruby.  For online
...
Revision $Id: README 198 2008-08-27 17:29:22Z grant $

(c) 2009 Ingres Corporation. All rights reserved.
